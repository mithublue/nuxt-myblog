  ...
  <section class="w-full md:w-2/3 flex flex-col items-center px-3">
    <template v-if="single.post">
      ...
      <div class="w-full flex pt-6">
        <NuxtLink v-if="single.post.previous" :to="'/post/'+single.post.previous.databaseId" class="w-1/2 bg-white shadow hover:shadow-md text-left p-6">
          <p class="text-lg text-blue-800 font-bold flex items-center"><i class="fas fa-arrow-left pr-1"></i> Previous</p>
          <p class="pt-2">{{ single.post.previous.title }}</p>
        </NuxtLink>
        <NuxtLink v-if="single.post.next" :to="'/post/'+single.post.next.databaseId" class="w-1/2 bg-white shadow hover:shadow-md text-right p-6">
